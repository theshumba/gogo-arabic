...
import { pixelBtnDark } from './grammarStyles.js';
...
export default function GrammarLesson({ lessonId, onBack }) {
  ...
  if (!lesson) {
    return (
      <div style={{
        ...
        background: COLORS.beige, fontFamily: FONTS.pixel, color: COLORS.dark,
      }}>
        <div>Lesson not found</div>
        <button onClick={onBack} style={{ ...pixelBtnDark, marginLeft: '20px' }}>Back</button>
      </div>
    );
  }
  ...
  switch (stage) {
    case 'explanation':
      return <ExplanationStage lesson={lesson} formatArabic={formatArabic} onBack={onBack} onNext={() => setStage('examples')} />;
    case 'examples':
      return <ExamplesStage lesson={lesson} formatArabic={formatArabic} onBack={() => setStage('explanation')} onNext={() => setStage('rules')} />;
    case 'rules':
      return <RulesStage lesson={lesson} onBack={() => setStage('examples')} onNext={() => setStage('exercises')} />;
    case 'exercises':
      return (
        <ExerciseStage
          lesson={lesson} currentIndex={currentExerciseIndex} score={exerciseScore}
          answers={exerciseAnswers} matchedPairs={matchedPairs} selectedMatchIndex={selectedMatchIndex}
          showFeedback={showFeedback} feedbackMessage={feedbackMessage} formatArabic={formatArabic}
          onAnswerSelect={handleAnswerSelect} onMatchSelect={handleMatchSelect} onNext={handleNext} onQuit={onBack}
        />
      );
    case 'quiz':
      return (
        <QuizStage
          lesson={lesson} currentIndex={currentQuizIndex} score={quizScore}
          answers={quizAnswers} showFeedback={showFeedback} feedbackMessage={feedbackMessage}
          onAnswerSelect={handleAnswerSelect} onNext={handleNext} onQuit={onBack}
        />
      );
    case 'complete':
      return <CompleteStage lesson={lesson} exerciseScore={exerciseScore} quizScore={quizScore} onBack={onBack} />;
    default:
      return null;
  }
}
